...
While having many different data interpreters and data producers is valuable, the identity layer must be narrow to serve its purpose. If individuals have a unique identity for every interaction they have with the World Republic, it is much more difficult for data interpreters to make judgments about them and user interfaces to unify all the relevant signals about a single individual.

As such, the World Republic Foundation is taking an opinionated approach to set a common standard for representing individuals in the World Republic. This standard will be used in initiatives run by the Foundation, including Retroactive Public Goods Funding (Retro Funding) and Citizen selection, and builders are encouraged to adopt the same principles to collectively build up identity within the World Republic.

### People and projects

There are two entities in the Republic: people and projects. People are unique human individuals and are associated with a particular [Farcaster account id](https://docs.farcaster.xyz/learn/what-is-farcaster/accounts). Projects are things created by individuals or groups of people, and are identified by an attestation UID from the [project schema](/identity/schemas#project-identifier). 

### People

Historically, onchain apps have managed user identity via an Ethereum address. Another wave of apps has prioritized user experience and embraced Web2 sign-in methods, or Web2.5 methods that create Ethereum accounts for users. None of these approaches satisfies the needs of the World Republic. 

The Republic needs a shared user identifier that persists even with key rotation, is permissionless, onchain and can be adopted by any builder or application and used across many applications. We plan to use this identifier to build a rich social network of World Republic builders, users, representatives, citizens and other participants in the Republic. Instead of building a new 'Optimist Id' we made the decision to use Farcaster accounts for this purpose. 
...
* The Farcaster protocol already has a vibrant social graph - bootstrapping a new social network for the World Republic would be difficult and redundant
...
* A specific contribution made to the World Republic, like a Dune Dashboard, is a project
...
The identity layer of the World Republic isn't complete without persistent project identity. As part of Retro Funding 4, we introduced the concept of an evergreen project that is represented onchain by the [Project Attestation](/identity/schemas#project-identifier). The attestation UID is the project identifier. Project metadata is stored off-chain and updated via the [Project Metadata Attestation](/identity/schemas#project-metadata).
...
Projects can have a nested structure via the 'parent project' field on the project metadata attestation. This creates a highly flexible architecture that can represent all types of contributions to the World Republic.
...
The purpose of this document is to outline the World Republic Foundation's approach to individual and project identity within the Republic, and invite builders to contribute to building out these fundamental building blocks. If you are a builder and you are looking for ways to add value to the Republic, check out the [Builder's Ideas List](https://contribute.optimism.io). To participate in the conversation about this topic, head over to the [Optimism Governance Forum](https://gov.optimism.io). For specific questions not answered here, email identity@optimism.io.